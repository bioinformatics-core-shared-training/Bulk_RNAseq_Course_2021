...
In this case many of the problems with the annotation are due to the versionss XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
The code for doing this is XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
ggplot(shrinkTab.11, aes(x = log2(baseMean), y=logFC)) + 
    geom_point(aes(colour=FDR < 0.05), shape=20, size=2) +
...
#Shrink our values
ddsShrink.11 <- lfcShrink(ddsObj, coef="Status_lactate_vs_virgin")
shrinkTab.11 <- as.data.frame(ddsShrink.11) %>%
    rownames_to_column("GeneID") %>% 
    left_join(ensemblAnnot, "GeneID") %>% 
    rename(logFC=log2FoldChange, FDR=padj)
...
>
> (a) Create a new column of -log10(pvalue) values in shrinkTab.11
>
...
ha1 = HeatmapAnnotation(df = colData(ddsObj.interaction)[,c("Group", "TimePoint")])
