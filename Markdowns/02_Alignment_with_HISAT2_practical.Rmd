...
>    There should be file called `Mus_musculus.GRCm38.chr1.fa.g`. This is the
...
>
>    This is where we will create our chr1 index.
...
* with the flags `-1` and `-2` to give two fastq files when we have paired end 
  reads
* with the `-U` flag to just give one fastq if our reads are single end (as is 
  the case today)
