...
BRIEF DESCRIPTION OF THE DATASET - paper, GEO, SRA

Please see extended material for instructions on 
...
BREIF DESCRIPTION OF EXPERIMENT - SAMPLE GROUPS, REPS etc.
...
sampleinfo <- read_tsv("meta_data/samplesheet.tsv", col_types = c("cccc")) %>% 
...
# This is named vector - values are the quant.sf files, names are SampleNames

files <- list.files("salmon", pattern = ".sf$", recursive = TRUE, full.names = TRUE)
files <- set_names(files, str_extract(files, "SRR7657[0-9]+"))
files <- files[sampleinfo$SampleName]
...
head(txi$counts)
```
```{r saveData, echo=FALSE, eval=FALSE}
...
sampleinfo <- mutate(sampleinfo, Group=case_when(
                                          SampleName=="SRR7657882" ~ "Control",
                                          SampleName=="SRR7657873" ~ "Test", 
                                          TRUE ~ Group))
...
**We can save the txi data object to use later**
