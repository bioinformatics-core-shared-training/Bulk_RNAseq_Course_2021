...
approach has been developed over the past few years has advantages of being
faster and more lightweight than full alignment (they can be run on a laptop
rather than needing a high performance cluster). In addition these methods give
more accurate estimates of gene expression at the transcript level and 
incorporate corrections for GC-content bias and sequence bias. 
They are also able to quantify multimapping reads in a more pragmatic manner
than the standard alignment/counting approaches giving more accurate counts, 
particularly for genes beloning to families of genes with very similar 
sequences.

There are various tools that use this approach such at `Kallisto`, `Sailfish` 
and `Salmon`; we will use `Salmon`. 

You can find the full manual here:

https://salmon.readthedocs.io/en/latest/salmon.html#using-salmon

This provides much more information about various options and the reasons behind
them.

# 1. Indexing the transcriptome for Salmon

As with the alignment step, we will need a reference to map the reads to, 
however, in this case we will be mapping to the transcriptome rather than the
genome. The transcriptome reference is a fasta file containing sequences
for all transcripts. Before we can run `Salmon` we need to create an index,
just as we did for `HISAT2`. For common species is possible to [download](http://refgenomes.databio.org/)
pre-generated Salmon indexes, however, it is worth knowing how to create your 
own. This process is more computationally intentensive and will not run on 
...
be erroneously counted against genes.
...
out differential gene expression analysis in R, we will want to summarise this
to the gene level. To do this we need a table that links transcript IDs to gene 
IDs. We have already done this for you, but for reference the code below was 
used to generate this table from the sequence headers in the transcriptome
reference file.

```
#### YOU DO NOT NEED TO RUN THIS CODE ####
echo -e "TxID\tGeneID" > references/tx2gene.tsv
