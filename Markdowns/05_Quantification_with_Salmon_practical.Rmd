...
just genes on Chromosome 14. We also need to include the genomic reference as 
decoy, so we also have this with only Chr 14.
...
Compare alignment rate (reads aligned to transcriptome) to the percentage of
UTR/coding in the Alignment QC report.

## Exercise 4 - Make transcript to gene table

We need this to summarise the quantification to gene level when loading into R

```
echo -e "TxID\tGeneID" > salmon_output/tx2gene.tsv
zcat references/Mus_musculus.GRCm38.cdna.all.fa.gz | 
  grep "^>" |
  tr ' ' '\t' |
  cut -f 1,4 |
  sed -e 's/^>//' -e 's/gene://' -e 's/\.[0-9]*$//' \
  >> salmon_output/tx2gene.tsv
```
